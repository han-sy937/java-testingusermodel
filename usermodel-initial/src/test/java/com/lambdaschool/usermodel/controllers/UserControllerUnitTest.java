...
//        roleService.deleteAll();
        ...
        u2.getUseremails()
                .add(new Useremail(u2,
                        "hops@mymail.local"));
        u2.getUseremails().get(1).setUseremailid(18);
        u2.getUseremails()
                .add(new Useremail(u2,
                        "bunny@email.local"));
//        u1.getUseremails().get(2).setUseremailid(19);
        userList.add(u2);
    ...
    public void getUserById() throws Exception{
        ...
        Mockito.when(userService.findUserById(7))
    ...
    public void getUserLikeName() throws Exception{
        String apiUrl = "/users/user/name/like/miss";
        Mockito.when(userService.findByNameContaining("miss")).thenReturn(userList);
        RequestBuilder rb = MockMvcRequestBuilders.get(apiUrl).accept(MediaType.APPLICATION_JSON);
        MvcResult r = mockMvc.perform(rb).andReturn();
        String tr = r.getResponse().getContentAsString();
    ...
    public void addNewUser() throws Exception{
        ...
        Role r2 = new Role("user");
        Role r3 = new Role("data");
        User u2 = new User("misspiggy",
                "1234567",
                "piggy@lambdaschool.local");
        u2.getRoles()
                .add(new UserRoles(u2, r2));
        u2.getRoles()
                .add(new UserRoles(u2, r3));
        u2.getUseremails()
                .add(new Useremail(u2,
                        "sesame@mymail.local"));
        u2.getUseremails().get(0).setUseremailid(27);
        u2.getUseremails()
                .add(new Useremail(u2,
                        "hops@mymail.local"));
        u2.getUseremails().get(1).setUseremailid(28);
        u2.getUseremails()
                .add(new Useremail(u2,
                        "bunny@email.local"));
        u2.getUseremails().get(2).setUseremailid(29);
        ObjectMapper mapper = new ObjectMapper();
    ...
    public void deleteUserById() throws Exception{
        String apiUrl = "/users/user/14";

        RequestBuilder rb = MockMvcRequestBuilders.delete(apiUrl, 14)
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON);
        mockMvc.perform(rb)
